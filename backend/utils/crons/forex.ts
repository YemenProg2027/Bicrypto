...
async function handleInvestmentUpdate(investment, user, roi, investmentResult) {
  ...
  try {
    ...
    // IMPORTANT: We return the principal + ROI on WIN, nothing on LOSS, principal only on DRAW
...
function calculateNewBalance(balance, amount, roi, result) {
  // Because we already subtracted 'amount' at creation time,
  // we only ADD back the principal if it's a WIN or DRAW.
  // For a LOSS, user doesn't get principal back, so do nothing.
  switch (result) {
    case "WIN":
      // Return principal + ROI
      return balance + amount + roi;
    case "LOSS":
      // Principal already taken, do not subtract again
      return balance;
    case "DRAW":
      // Return principal, no profit or loss
...
async function updateInvestmentStatus(investmentId, result, roi, transaction) {
  try {
    ...
    return await models.forexInvestment.findByPk(investmentId, {
      include: [
